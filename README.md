...
- `nixosModules.{immich,rauthy,vikunja,forgejo}` (plus `default = rauthy`, a back-compat alias
  retained only during the canix migration)
- `lib.{immich,rauthy,vikunja,forgejo}` — `usersFromKanidmPersons` for Immich/Rauthy,
  plus service-specific `kanidmOAuth2System` helpers for Immich, Vikunja, and Forgejo
